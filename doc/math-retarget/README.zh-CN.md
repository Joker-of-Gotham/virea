...
$$
\mathcal{F}=\{\mathrm{hips}\}\cup\mathcal{C}\cup\mathcal{H}
$$

其中 $|\mathcal{C}|=21$，$|\mathcal{H}|=30$。父节点映射为 $\pi(j)$，primary child 映射为 $\chi(j)$，target rest offset 为 $\bar{o}_j$，source rest offset 为 $o_j^{\mathrm{src}}$。
...
$$
\mathrm{BABEL\ motion\ carrier}=\mathrm{AMASS/SMPL\text{-}H\ poses}
$$

BABEL 只增加 annotation：

$$
\mathcal{A}_{\mathrm{BABEL}}\nrightarrow q_t^j
$$

GRAB 与 Motion-X 放在同一篇 SMPL-X 文档，但 profile 分开，因为：

$$
B_{\mathrm{GRAB}}=\texttt{z\_up\_to\_y\_up},\qquad
B_{\mathrm{MotionX}}=\texttt{identity\_y\_up}
$$

BEAT 单独写，是因为虽然它进入同一个 `AxisAngleBody22Codec` 类，但配置是：

$$
\mathrm{world\_basis}_{\mathrm{BEAT}}=\texttt{identity\_y\_up}
$$

而不是 AMASS/BABEL 的：

$$
\mathrm{world\_basis}_{\mathrm{AMASS}}=\texttt{z\_up\_to\_y\_up}
$$
